...
<head>
    ...
    <div class="produk-container">
        ...
        <?php
        // Cek ID Produk Galon (Asumsi ini adalah ID 4)
        $GALON_BERISI_ID = 4;
        ...
        ?>
                    ...
                    <table class="produk-table" id="tabel-produk">
                        <thead>
                            <tr>
                                ...
                                <?php if ($level_lowercase == 'kasir'): /* HANYA Kasir yang fokus ke stok */ ?>
                                    <th style="width: 150px;">Aksi Stok</th>
                                <?php endif; ?>

                                <?php if ($level_lowercase == 'admin'): /* HANYA Admin untuk manajemen data produk */ ?>
                                    <th class="actions">Aksi Produk</th>
                                <?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            if ($tampil && mysqli_num_rows($tampil) > 0) {
                                while ($r = mysqli_fetch_assoc($tampil)) {
                                    ...
                                    $is_disabled = ($id_produk == $GALON_BERISI_ID);

                                    $link_edit_produk = "index.php?menu=" . htmlspecialchars($menu) . "&act=edit&id_produk=" . $id_produk;
                                    $link_hapus = "./data.php?menu=" . htmlspecialchars($menu) . "&act=batal&id_produk=" . $id_produk;
                                    $link_edit_stok = "index.php?menu=" . htmlspecialchars($menu) . "&act=tambah_stok_form&id_produk=" . $id_produk;
                                    ...
                                    // Kolom Aksi Stok (HANYA Kasir)
                                    if ($level_lowercase == 'kasir') {
                                        echo "<td data-label='Aksi Stok' class='add-stock-cell' style='text-align: center;'>";
                                        if ($is_disabled) {
                                            echo "<button class='btn btn-xs btn-disabled' disabled><i class='fas fa-ban'></i> Otomatis</button>";
                                            echo "<span class='info-text'>(Stok via pesanan)</span>";
                                        } else {
                                            // Kasir bisa edit stok (tambah/kurang)
                                            echo "<a href='" . $link_edit_stok . "' class='btn btn-xs btn-primary' title='Edit Stok'><i class='fas fa-box-open'></i> Edit Stok</a>";
                                        }
                                        echo "</td>";
                                    }

                                    // --- KOLOM AKSI PRODUK (HANYA ADMIN) ---
                                    if ($level_lowercase == 'admin') {
                                        echo "<td class='actions'>";
                                        if ($is_disabled) {
                                            // Admin dilarang mengedit atau menghapus produk inti
                                            echo "<button class='btn btn-xs btn-disabled' title='(Otomatis)' disabled><i class='fas fa-edit'></i></button> ";
                                            echo "<button class='btn btn-xs btn-disabled' title='(Tidak bisa dihapus)' disabled><i class='fas fa-trash'></i></button>";
                                        } else {
                                            // Admin mengedit detail produk (Nama, Harga, Jenis)
                                            echo "<a href='" . $link_edit_produk . "' class='btn btn-xs btn-success' title='Edit Detail Produk'><i class='fas fa-edit'></i></a> ";
                                            // Admin Hapus
                                ...
                                // Perhitungan colspan diperbaiki
                                $base_cols = 4;
                                $cols_add_stock = ($level_lowercase == 'kasir') ? 1 : 0;
                                $cols_action = ($level_lowercase == 'admin') ? 1 : 0;
                                $colspan = $base_cols + $cols_add_stock + $cols_action;
            ...
            // =======================================================
            // KASUS: FORMULIR TAMBAH STOK (Akses Kasir/Admin)
            // =======================================================
            case "tambah_stok_form":
                ...
                // Cek jika ID 4 (Galon), tidak boleh tambah stok manual
                if ($id_produk_stok == $GALON_BERISI_ID) {
                    echo "<div class='alert alert-warning' style='text-align:center; max-width: 600px; margin: 20px auto;'>";
                    echo "<h4>Aksi Dibatasi</h4>";
                    echo "<p>Stok 'Galon Berisi' (ID 4) diperbarui secara otomatis melalui transaksi pesanan dan Cron Job.</p>";
            ...
            // =======================================================
            // KASUS: TAMBAH PRODUK BARU (HANYA Admin)
            // =======================================================
            case "tambah":
            ...
            ?>
                <div class="form-section">
                    ...
                    <form method="POST" action="./data.php?menu=<?php echo htmlspecialchars($menu); ?>&act=input">
                        ...
                        <div class="form-group">
                            <label for="stock_display">Stok Awal</label>
                            <div class="input-wrapper">
                                <span>0</span>
                                <input type="hidden" id="stock" name='stock' value="0" />
                            </div>
                            <small>Stok akan disetel 0. Kasir wajib mengisinya melalui 'Aksi Stok' setelah produk dibuat.</small>
                        </div>
            ...
            // =======================================================
            // KASUS: EDIT PRODUK DETAIL (HANYA Admin)
            // =======================================================
            case "edit":
                // Akses ketat: HANYA Admin yang dapat mengedit detail produk
                if ($level_lowercase != 'admin') {
                    echo "<div class='alert alert-danger'>Akses ditolak. Hanya Admin yang dapat mengedit detail produk.</div>";
                    break;
                }

                $id_edit = isset($_GET['id_produk']) ? (int)$_GET['id_produk'] : 0;
                if ($id_edit <= 0) {
                    echo "<p style='color:red;'>ID Produk tidak valid.</p>";
                    break;
                }
            ...
            ?>
                <div class="form-section">
                    <h2>Edit Detail Produk: <?php echo htmlspecialchars($r['nama_produk']); ?></h2>
                    <form method="POST" action="./data.php?menu=<?php echo htmlspecialchars($menu); ?>&act=update">
                        ...
                        <input type="hidden" name="stok_lama" value="<?php echo htmlspecialchars($r['stock']); ?>">
                        ...
                        <div class="form-group">
                            <label for="stock_display">Stok Saat Ini</label>
                            <div class="input-wrapper">
                                <span><?php echo htmlspecialchars($r['stock']); ?></span>
                                <input type="hidden" id="stock" name="stock" value="<?php echo htmlspecialchars($r['stock']); ?>">
                            </div>
                            <small>Stok hanya dapat diubah melalui tombol 'Aksi Stok' di daftar produk oleh Kasir.</small>
                        </div>

                        <div class="form-actions">
                            <button type='button' class="btn btn-secondary" onclick='window.location.href="index.php?menu=<?php echo htmlspecialchars($menu); ?>"'>Batal</button>
                            <button type='submit' class="btn btn-primary" onclick="return confirm('Apakah perubahan data produk ini sudah benar dan yakin ingin diupdate?');">Update Produk Detail</button>
                        </div>
                    </form>
                </div>
                <br><br>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (form) {
                    form.addEventListener('submit', function(event) {
                        // Cek elemen yang terlihat dan dapat diedit
                        ...
                        // Validasi Stok Awal (Hanya di case tambah, karena input stock adalah hidden)
                        // Karena kita set ke 0 di PHP, validasi ini tidak lagi diperlukan di JS untuk case 'tambah'.
