 ...
 switch($_GET[act])
{
 ...
 </form><form action=?menu=pengiriman method=POST>
 ...
 <table width=100% STYLE=BORDER-COLLAPSE:COLLAPSE; border=0 bgcolor=#333333>
		 ...
		 {
  	     $tampil=mysqli_query($conn,"select * from faktur_pesan where status=2 order by no_order desc");
		 }
		 else
		 {
		 $tampil=mysqli_query($conn,"select * from faktur_pesan where kd_pesan like '%$_POST[kd_pesan]%' and status=2 order by no_order desc");
		...
		echo "
		 ...
		 <td>$ket
		 <td> 
 	      <a href=\"./data.php?menu=pengiriman&act=update&kd_pesan=$r[no_order]\"onClick=\"return confirm('Update Pengiriman (&nbsp;&nbsp;Kode Pemesanan :$r[no_order]&nbsp;&nbsp;) ?')\"><center>Update</a>  
